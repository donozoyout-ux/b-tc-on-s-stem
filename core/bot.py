...
class TradingBot:
    ...
    def __init__(self):
        ...
        self.virtual_balance: float = 10000.0  # Başlangıç Sanal Sermaye: 10,000 USDT
        ...
        self.add_log("INFO", "Trading Bot Paper Trading Motoru ilklendirildi (10,000 USDT).")
    ...
    async def start(self) -> None:
        ...
        """
        logger.info("Kripton Trading Bot Paper Trading Motoru başlatılıyor...")
        self.add_log("INFO", "Bot başlatılıyor ve piyasa istemcisi kuruluyor...")
        await self.exchange.initialize()
        self.is_running = True

        mode_str = "Sanal Para (Paper Trading - 10,000 USDT)" if settings.TEST_MODE else "GERÇEK HESAP"
    ...
    async def run_loop(self) -> None:
        """
        Sürekli Tetikte Kalma ve Strateji Kontrol Döngüsü (7/24).
        """
        await self.start()
        ...
        while self.is_running:
            try:
                ...
                # 1. Canlı mum (OHLCV) verisini çek ve piyasa durumunu tara
                ...
                if df is not None and len(df) > 0:
                    ...
                    ema_status = "BULLISH" if fast_ema > slow_ema else "BEARISH"
                    stoch_status = f"K:{stoch_k:.1f}/D:{stoch_d:.1f}"
                    ...
                    # 3. Pozisyon yoksa ve sinyal onaylandıysa yeni pozisyon aç
                    elif signal in [SignalType.LONG, SignalType.SHORT]:
                        action_str = await self._open_position(signal.value, current_price, atr)
                    ...
                    log_output = (
                        ...
                        f"[PİYASA ANALİZİ]: EMA Trend: {ema_status} | Stoch RSI: {stoch_status} | ATR: {atr:.2f} | Fiyat: ${current_price:.2f}\n"
                        f"[AKSİYON]: {action_str}\n"
                        f"[AÇIK POZİSYONLAR]: {open_pos_summary}\n"
                        f"[İŞLEM GEÇMİŞİ SON 5]: {history_summary}\n"
                    )
                    logger.info(log_output)
                    self.add_log("INFO", f"Tarama @ ${current_price:.2f} | Aksiyon: {action_str}")
    ...
    async def _open_position(self, signal_type: str, current_price: float, atr: float) -> str:
        """
        Gerçekçi simülasyon şartlarında yeni paper trading pozisyonu açar.
        """
        ...
        position_obj = {
            ...
            "estimated_fees": round(entry_fee * 2, 2), # tahmini giriş+çıkış
    ...
    async def _close_position(self, pos: Dict[str, Any], reason: str, exit_price: float) -> None:
        """
        Açık pozisyonu net komisyon ve kayma düşerek kapatır, bakiyeyi günceller.
        """
        signal_type = pos["side"]
        entry_price = pos["entry_price"]
        ...
        # Sanal bakiyeyi Net PnL ile güncelle
